define([
  ...
  return declare([ViewerBase], {
    ...
    postCreate: function () {
      ...
      var treeSort = this.state.search.match(/treeSort=.+?(?=&|$)/);
      ...
      if (treeSort && !isNaN(treeSort.index)) {
        treeSort = treeSort[0].split('=')[1];
        treeSort = (treeSort === 'true') || (treeSort === '1');
      } else { treeSort = false; }
      ...
      if (folderCheck && !isNaN(folderCheck.index)) {
        ...
        WorkspaceManager.getObjects([objPath]).then(lang.hitch(this, function (objs) {
          ...
          if (typeof obj.data == 'string') {
            ...
            // If the "treeSort" parameter was included in the URL, the file was
            // generated by the TreeSort service. Visualizations are derived from
            // the properties already present in its tree.
            if (treeSort) {
              if (!treeDat.options) { treeDat.options = {}; }
              treeDat.options.enableVisualizations = true;
            }
